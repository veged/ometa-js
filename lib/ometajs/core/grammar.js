...
AbstractGrammar.matchAll = function matchAll(source, rule, args) {
  ...
  if (!grmr._rule(rule, false, args)) {
    if (grmr._error.offset >= 0) {
      throw new Error('failed to match ' + grmr._error.rule + ' rule at:' +
                      grmr._error.offset);
    } else {
    }
  }
...
//
//
AbstractGrammar.prototype._rule = function rule(name, nocache, args) {
  ...
  if (name === 'token') {
    ...
    return this._atomic(function() {
      ...
      if (!this._invoke(this.constructor, name, nocache)) return false;
  ...
  return this._invoke(this.constructor, name, nocache, args);
};
...
//
//
AbstractGrammar.prototype._super = function _super(name, nocache, args) {
  var super_ = this.constructor.super_,
      constructor = this.constructor,
      res;

  this.constructor = super_;
  res = this._invoke(super_, name, nocache, args);
...
AbstractGrammar.prototype.space = function space() {
  return this._fnMatch(function(v) { return /^\s$/.test(v) });
};
