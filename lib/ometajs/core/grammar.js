...
AbstractGrammar.prototype.seq = function seq(str) {
  if (this.type === 'string') {
    var head = this.source.slice(this.offset, this.offset + str.length);
...
AbstractGrammar.prototype._rule_token = function _rule_token() {
  if (!this.rule('_rule_anything')) return false;

  return this.cache('::token', this.intermediate, function() {
    var token = this.intermediate;

    // Match whitespace after token and close 'chars'
    if (this.seq(token) && this.rule('_rule_spaces')) {
...
AbstractGrammar.prototype._rule_seq = function _rule_seq() {
  if (!this.rule('_rule_anything')) return;
