...
AbstractGrammar.prototype.rule = function rule(name, args) {
  ...
  if (args) {
    ...
    return this.cache(this.constructor.name, name, invoke);
  }
};
...
AbstractGrammar.prototype._super = function _super(name, args) {
  function invoke() {
    return this.constructor.super_.prototype['_rule_' + name].call(this);
  };

  if (args) {
    return this.simulate(args, invoke);
  } else {
    return this.cache(this.constructor.super_.name, name, invoke);
  }
};
