...
AbstractGrammar.prototype._invoke = function _invoke(grmr, rule, args) {
  function invoke() {
    ...
    return grmr.prototype[rule].call(this);
  }
...
AbstractGrammar.prototype.rule = function rule(name, args) {
  ...
  if (name === '_rule_token') {
    var flat = this.history.length === 0 && this.type === 'string',
        offset = this.offset;
    ...
    if (flat) {
      ...
      if (cache !== undefined) {
        if (cache.token === args[0]) {
          ...
          this.setIntermediate(cache.value, true);
          return true;
        } else {
          return false;
        }
    ...
    return this.atomic(function() {
      ...
      if (!this._invoke(this.constructor, name)) return false;

      // Get result
      var pair = this.getIntermediate();
      ...
      if (pair[0] === args[0]) {
        this.setIntermediate(pair[1], true);
        return true;
      } else {
        return false;
      }
...
AbstractGrammar.prototype.fromTo = function fromTo(from, to) {
  var head = this.source.slice(this.offset, this.offset + from.length);
  if (head !== from) return false;

  var t = this.source.indexOf(to, this.offset);

  if (t === -1) return false;

  t += to.length;

  var value = this.source.slice(this.offset, t);
...
AbstractGrammar.prototype.seq = function seq(str) {
  if (this.type === 'string') {
    ...
    if (len < 4) {
      ...
      var head = this.source.slice(this.offset, this.offset + len);
...
//
AbstractGrammar.prototype.word = function word() {
  if (this.type === 'string') {
    var match = this.source.slice(this.offset).match(/^[^\s]+/);
    if (match === null) return false;
    this.skip(match[0].length);
    this.setIntermediate(match[0]);
    ...
    while (!this.isEnd() && !(/\s/.test(current = this.current()))) {
      this.skip();
      chars.push(current);
    }
    ...
    this.setIntermediate(chars.join(''));
...
AbstractGrammar.prototype.any = function any(fn) {
  ...
  while (!this.isEnd() && fn.call(this)) {
    list.push(this.getIntermediate());
  }
...
AbstractGrammar.prototype._rule_token = function _rule_token() {
  ...
  var token = this.getIntermediate();

  // Match whitespace after token and close 'chars'
  if (this.rule('_rule_spaces')) {
    this.setIntermediate([token, token], true);
    return true;
  } else {
    this.setIntermediate(undefined);
    return false;
...
AbstractGrammar.prototype._rule_space = function _rule_space() {
  return this.fnMatch(function(v) { return /^\s$/.test(v) });
};
...
AbstractGrammar.prototype._rule_fromTo = function _rule_fromTo() {
  this.skip();
  var from = this.getIntermediate();
  this.skip();
  var to = this.getIntermediate();
...
AbstractGrammar.prototype._rule_char = function _rule_char() {
  return this.fnMatch(function(curr) {
...
AbstractGrammar.prototype._rule_listOf = function _rule_listOf() {
  this.skip();
  var rule = '_rule_' + this.getIntermediate();

  this.skip();
  var sep = this.getIntermediate();

  if (!this.rule(rule)) {
    this.setIntermediate([], true);
    return true;
  }
  var list = [this.getIntermediate()];
  ...
  this.setIntermediate(list, true);
