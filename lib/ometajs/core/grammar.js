var ometajs = require('../../ometajs'),
    ...
    util = require('util');
    Cons = ometajs.utils.Cons,
    ArrCons = ometajs.utils.ArrCons,
    SourceMap = ometajs.utils.SourceMap;
...
AbstractGrammar.matchAll = function matchAll(source,
  ...
  var result = grmr._getIntermediate(),
      sresult = result instanceof Cons ? result.toString() : result;

  if (options && options.sourcemap) {
    assert(result instanceof Cons);

    var map = new SourceMap(result);
    options.sourcemap.call(this, map);
  }
...
AbstractGrammar.prototype._arrcons = function arrcons(arr) {
  return new ArrCons(this._info, arr);
};

AbstractGrammar.prototype._cons = function cons(value) {
  if (value instanceof Cons) return value;

  return new Cons(this._source._info, value + '');
};
