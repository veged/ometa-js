...
function AbstractParser(source) {
  ...
  this.intermediateLazy = undefined;
...
AbstractParser.prototype._save = function _save() {
  return {
    ...
    offset: this.offset,
    type: this.type,
    history: this.history.length > 0 ? this.history.slice() : [],
    intermediate: this.intermediate,
    intermediateLazy: this.intermediateLazy,
    result: this.result,
...
AbstractParser.prototype._load = function _load(state, values) {
  ...
  if (values) {
    ...
    this.intermediateLazy = state.intermediateLazy;
...
AbstractParser.prototype.cache = function cache(grammar, rule, body) {
  ...
  var key = this.history.length > 0 ?
          this.history.concat(grammar).join(':')
          :
          grammar,
      // Fast offset level
      cacheLevel = this._cache[this.offset] || (this._cache[this.offset] = {}),
      cacheDir = cacheLevel[key] || (cacheLevel[key] = {}),
      cache,
      res;
...
AbstractParser.prototype.atomic = function atomic(body, lookahead) {
  ...
  if (!status || lookahead) {
    ...
    this.setIntermediate(null, null, [source, start, end]);
  }
...
//
AbstractParser.prototype.getIntermediate = function getIntermediate() {
  if (this.intermediateLazy) {
    var lazy = this.intermediateLazy;
    this.intermediate = lazy[0].slice(lazy[1], lazy[2]);
  }
...
//
AbstractParser.prototype.match = function match(value) {
...
//
AbstractParser.prototype.fnMatch = function match(fn) {
