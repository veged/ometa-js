...
AbstractParser.prototype._lastError = function _lastError(rule) {
  if (this._error.offset <= this._offset) {
    this._error.rule = rule;
    this._error.offset = this._offset;
    this._error.source = this._source;

    if (this._debug) this._error.stack = new Error();
  }
};
...
AbstractParser.prototype._cache = function cache(grammar, rule, body) {
  ...
  var key = this._history ? [this._history, grammar].join(':') : grammar,
      // Fast offset level
      cacheLevel = this.__cache[this._offset] || (this.__cache[this._offset] = {}),
...
//
AbstractParser.prototype._fnMatch = function match(fn) {
...
//
AbstractParser.prototype._skip = function skip(num) {
  if (this._type === 'simulate') {
    this._source.skip(num);
  } else {
    if (num) this._offset += num - 1;
    this._setIntermediate(this._current());
    this._offset++;
  }
