...
AbstractParser.prototype._atomic = function atomic(body, lookahead) {
  ...
  if (simulated && this._type === 'simulate') {
    var simulateOffset = this._source.offset;
  }
  ...
  if (!status || lookahead) {
    ...
    // Restore simulate's position
    if (simulated && simulateOffset !== undefined) {
      source.offset = simulateOffset;
    }
