...
// Also will temporary nullify `result` property
//
AbstractParser.prototype.cache = function cache(rule, body) {
  ...
  var key = this.history.concat(this.offset, rule).join(':'),
  ...
  if (cache) {
    ...
    if (cache.status === 'rec-detection') {
      cache.detected = true;
    }

    // If result is positive - move position to cached one
    if (res = cache.result) this._load(cache.state);
  } else {
    this._cache[key] = cache = {
      status: 'rec-detection',
      detected: false,
      result: false,
      state: this._save()
    };
    ...
    if (cache.detected) {
      ...
      do {
        ...
        if (body.call(this)) break;
      } while (this.source !== source || this.offset !== offset);
    }
...
// #### @value {any}
// #### @result {Boolean} value won't be overwritten by .atomic()
...
AbstractParser.prototype.exec = function exec(fn) {
  this.setIntermediate(fn.call(this), true);
