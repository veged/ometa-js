...
AbstractParser.prototype.leave = function leave(ok) {
  ...
  if (ok) {
    ...
    state.intermediate = state.source.slice(state.offset, offset);
    state.offset = offset;
...
AbstractParser.prototype.skip = function skip() {
  ...
  // Out of bounds check
  if (state.source.length < ++state.offset) state.source = undefined;
...
AbstractGrammar.prototype._rule_token = function _rule_token() {
  ...
  // Leave simulated data
  if (!this.close('simulate')) return false;
