...
//
// ### function parseExpression (space)
// #### @space {Boolean} match space
// Parser's routine
//
Parser.prototype.parseExpression = function parseExpression(space) {
  var token = this.token(null, null, space);

  if (matchToken(token, 'punc', '(')) {
    // ( expr | ... | expr )
    this.push(['group']);
    this.list('choice', 'punc', '|', function() {
    ...
    this.push(['list']);
    // [ expr expr expr ]
    this.list('list', 'space', null, function() {
...
//
// ### function parseHostExpression ()
// Parser's routine
//
Parser.prototype.parseHostExpression = function parseHostExpression() {
  var token,
      depth = 0,
      dived = false,
      code = [];
...
//
// ### function list (op, type, value, parse)
// #### @op {String}
// #### @type {String}
// #### @value {String|Array}
// #### @parse {Function}
// Parser's helper
//
Parser.prototype.list = function list(op, type, value, parse) {
  var token;
  do {
    if (token) {
