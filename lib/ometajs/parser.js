...
Parser.prototype.parseExpression = function parseExpression(type) {
  if (type === 'left') {
    ...
    // :name
    if (this.optional('punc', ':', true) && this.commit()) {
      arg = this.token('name');
      this.push(['arg', null, [], arg.value]);
    // rule(...args...) or rule(...args...):name
    } else if (arg = this.optional('name', null, true)) {
      this.commit();
      this.push(['arg', arg.value]);
      ...
      this.pop();
    // -> { javascript body }
    } else if (this.optional('punc', '->') && this.commit()) {
      this.push(['body']);
      this.parseHostExpression();
      this.pop();
    }
...
Parser.prototype.parseHostExpression = function parseHostExpression() {
  ...
  for (;;) {
    ...
    if (dived ? depth < 0 : matchToken(token, 'punc', ',')) {
      this.fail();
      break;
    }
