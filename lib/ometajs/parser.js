...
Parser.prototype.execute = function execute() {
  while (this.active) {
    if (this.state === 'empty') {
      ...
      while (this.active && !(end = this.optional('punc'))) {
      ...
      if (matchToken(end, 'punc', '|') || matchToken(end, 'punc', '||')) {
        this.commit();
        // Parse next expression
      } else if (matchToken(end, 'punc', ',')) {
        this.commit();
        // Parse next rule
        this.state = 'rule';
      } else if (matchToken(end, 'punc', '}')) {
        ...
        this.unexpected(end, 'punc', '| or || or , or }');
      }
    }
  }
...
Parser.prototype.parseRule = function parseRule() {
  ...
  // Allocate space for expressions
  this.push([]);
};
