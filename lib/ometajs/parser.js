...
Parser.prototype.parseExpression = function parseExpression() {
  ...
  function parseInvocation() {
    ...
    if (!space && self.maybe('punc', '(')) {
      ...
      self.push([]);
      self.parseUntil('punc', ')', function() {
        self.parseHostExpression([','], false, true);
        self.maybe('punc', ',');
      });
      self.accept();
      self.pop();
  ...
  for (;;) {
    ...
    if (space && depth <= 0 &&
        (matchToken(token, 'name') && this.lookahead('space') ||
         matchToken(token, 'space') && this.lookahead('name'))) {
      this.reject();
      break;
    }
  }

  this.push(expr ? code.join('') : utils.expressionify(code.join('')));
  this.pop();
};
...
Parser.prototype.reject = function reject() {
  if (this.pending.length > 0) {
    this.tokens.push(this.pending.pop());
  }
  return true;
};
