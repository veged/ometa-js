...
function objectThatDelegatesTo(obj, props) {
  // Regular case
  if (!(obj.prototype instanceof grammars.AbstractGrammar)) {
  ...
  adapter._genericApply = function genericApply(recv, rule, args) {
    ...
    if (grmr._rule(rule, false, args, recv === null ? undefined : recv)) {
      var res = grmr._getIntermediate();

      this.input.lst = grmr._source;
      this.input.idx = grmr._offset;
