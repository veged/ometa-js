...
// #### @fn {Function} that'll collect body
//
...
Compiler.prototype.compileOperation = function compileOperation(op, context) {
  ...
  switch(op[0]) {
    case 'match':
      ...
      ir.enter('code', function() { this.push('this._rule_', op[2], '()') });
      break;
    case 'call':
      if (op[1] !== null) throw new Error('Not implemented yet');
      ir.enter('and', function() {
        this.enter('code', function() {
          this.push('this.sumulate([');
    ...
    case 'arg':
      ir.enter('and', function() {
        ...
        this.enter('code', function() {
          this.push('this.set(', JSON.stringify(op[2]), ')');
        });
      });
    ...
    case 'list':
      ir.enter('and', function() {
        this.enter('code', function() { context.enter() });
        this.enter('code', function() { this.push('this.open("list")') });

        for (var i = 1; i < op.length; i++) {
          self.compileOperation(op[i], context);
        }

        this.enter('code', function() { this.push('this.close()') });
        this.enter('code', function() { context.leave() });
      });
      break;
    case 'super':
